...
import com.ammar.walletconnectlibadd.databinding.ScreenMainBinding
...
class MainActivity : AppCompatActivity(), Session.Callback {
    ...
    private fun sessionApproved() = with(binding) {
        uiScope.launch {
            ...
            screenMainConnectButtonSequence.visibility = View.GONE
            screenMainConnectButtonQrCode.visibility = View.GONE
            screenMainDisconnectButton.visibility = View.VISIBLE
            if (isAppConnection) screenMainTxButton.visibility = View.VISIBLE
        }
    }
    ...
    private fun sessionClosed() = with(binding) {
        uiScope.launch {
            ...
            screenMainConnectButtonSequence.visibility = View.VISIBLE
            screenMainConnectButtonQrCode.visibility = View.VISIBLE
    ...
    override fun onStart() = with(binding) {
        ...
        screenMainConnectButtonSequence.setOnClickListener {
            ...
            val webAddress = ExampleApplication.config.toWCUri()
            val encodedUrl: String =
                URLEncoder.encode(webAddress, "UTF-8").replace("=", "%3D").replace("&", "%26")
            val finalUrl = "https://sequence.app/wc?uri=" + encodedUrl
            i.data = Uri.parse(finalUrl)
            startActivity(i)
        }
        screenMainConnectButtonQrCode.setOnClickListener {
            isAppConnection = false
            ExampleApplication.resetSession()
            ExampleApplication.session.addCallback(this@MainActivity)
...
//    }
    private var qrDialog: WalletConnectQrDialog? = null

    private fun showWalletConnectQrCode() {
        if (qrDialog != null) qrDialog?.dismiss()
        WalletConnectQrDialog(ExampleApplication.config.toWCUri(), positiveClickClosure = {
